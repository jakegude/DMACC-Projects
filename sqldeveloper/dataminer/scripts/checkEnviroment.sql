...
BEGIN
  ...
  IF( v_version < '11.2.0.1') THEN
    RAISE_APPLICATION_ERROR(-20000, 'Not Supported Database Version.');
  ELSIF( SUBSTR(v_version,1,4) >= '12.2') THEN
    RAISE_APPLICATION_ERROR(-20000, 'Data Miner can not be installed on 12.2 DB.');
  END IF;
...
EXCEPTION
  WHEN invalid_userenv THEN  -- not a valid argument on db less than 12.1
    DBMS_OUTPUT.PUT_LINE('Validated database is not CD$ROOT.');  
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('NO_DATA_FOUND when attempting to query SYS_CONTEXT CON_NAME.');
